...
- `tests/weather/getWeather.test.js` — unit tests for the public `getWeather`. Includes the `API_KEY` guard, confirmation that it sends `timezone=auto` and targets the `standard` endpoint, and that it surfaces `timezone` plus an internal `localDay` per hour.
