...
src/db.js                    →  query / initDb                    lazy pg pool + idempotent devices schema (push path only)
src/notifications/apns.js    →  sendPush / StaleTokenError        APNs seam (apns2 imported nowhere else)
src/jobs/index.js            →  startJobs()                       in-process node-cron, hourly digest pass (single replica)
...
The HTTP API has four routes: the two below plus the push-path device routes (`PUT`/`DELETE /api/v1/devices/:deviceId` — see [Push path](#push-path--device-registration--daily-digest-issue-6c)). Treat the shapes below as the contract — the iOS client (Issue #5a) decodes against them.
...
### `src/db.js`
- Lazy `pg` pool over `DATABASE_URL` (requiring the module never opens a connection — only the first query does), `query` passthrough, idempotent `initDb()` creating the `devices` table. The push path's only state; `/rating` and `GET /health` never touch it.

### `src/notifications/`
- `apns.js` — the APNs seam; `apns2` is imported nowhere else (provider-specifics stop at the boundary). `createPushSender({ transport })` for tests + the production `sendPush(apnsToken, { title, body, payload })`; the real transport is built lazily from env on first send (`buildApnsConfig`: `APNS_KEY` = `.p8` PEM **content**, `\n`-escape tolerant; `APNS_KEY_ID`; `APNS_TEAM_ID`; topic `com.timeit.app`; sandbox host unless `NODE_ENV === 'production'`). APNs `Unregistered`/`BadDeviceToken` → typed `StaleTokenError` so callers delete the device row.

### `src/jobs/`
- `index.js` — `startJobs()`: `node-cron` `0 * * * *` wiring the digest job to the production dependencies (shared cache, real db, APNs singleton). Called by `app.js` **after** `listen`, on the single always-on replica.
- `dailyDigest.js` — `createDailyDigestJob({ db, getWeather, evaluateAll, sendPush, now })` → `runDigestPass()`. Selection, dedup, composition, and stale-token handling as described in the [Push path](#push-path--device-registration--daily-digest-issue-6c) section; per-device try/catch so one failure never stops a pass. Known cosmetic limitation: half-hour zones (e.g. Asia/Kolkata +05:30) render `ha`-style clock labels :30 off — the server-side twin of the iOS `TimeDeriver` limitation (STATUS §5).
...
- **In progress:** **[#6c device registration + daily digest](docs/issues/current/implement-spec-issue-6c-registration-and-digest.md)** — **backend built 2026-08-01** (branch `issue-6c-registration-and-digest`): Postgres `devices` schema + `initDb`, snapshot upsert/delete routes, shared `validateActivities`/error-envelope extractions, shared 60-min weather cache (now also serving `/rating`), `apns2` seam, hourly digest job. **Remaining:** owner's Railway/APNs manual steps and the iOS §9 opt-in/registration client (deferred — needs the APNs entitlement and the pbxproj currently in flight with the WeatherKit track); see the handoff in `docs/issues/current/`.
- **Next:** [#6d Perfect-window detector](docs/issues/current/implement-spec-issue-6d-perfect-window-detector.md) (hourly, Perfect-only, bucket-date dedup, buckets 0–1; reuses every piece of #6c infrastructure). #6a accounts/auth is CUT per ADR-0001.
