...
- `labels.js` — `hourLabel`/`rangeLabel`, the clock-label copy shared by both jobs (the server-side twin of the iOS `TimeDeriver`). Known cosmetic limitation: half-hour zones (e.g. Asia/Kolkata +05:30) render the `ha`-style labels :30 off (the iOS `TimeDeriver` shares it).
