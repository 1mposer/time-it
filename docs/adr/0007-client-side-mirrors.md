...
3. `HourQuality` (iOS, spec 14 §3 — unbuilt) ↔ `evaluateHour` (`src/decision/decision_engine.js`) — per-hour tier evaluation for the gradient card. Pinned by the spec 14 §3 invariant test against `RealBackendResponseFixture` (greenest run must coincide with the server's returned window).
