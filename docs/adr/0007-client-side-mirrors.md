...
2. `RangeText` (iOS) ↔ `src/jobs/labels.js` — clock-label copy (`hourText`/`hourLabel`, the same-meridiem collapse). `TimeDeriver.rangeLabel` routes through `RangeText.rangeText`, so the client has exactly one implementation. *(Amended 2026-08-29: `RangeText` had grown into an undeclared second mirror with a divergent spaced-dash dialect — the tight dialect won by owner ruling, the collapse rule was unified into `RangeText`, and both sides now pin one shared table: `tests/fixtures/clock-labels.json`, read by `tests/jobs/labels.test.js` and `RangeTextTests`.)* Shared known limitation: half-hour zones render `ha`-style labels :30 off.
3. `HourQuality` (iOS, spec 14 §3) ↔ `evaluateHour` (`src/decision/decision_engine.js`) — per-hour tier evaluation for the gradient card. Pinned by the spec 14 §3 invariant test against `RealBackendResponseFixture` (greenest run must coincide with the server's returned window — `HourQualityInvariantTests`). **Recorded asymmetry (2026-08-13 audit):** the tolerant decoder defaults an absent `dustAlert`/`seaWarning` to `false`, so a contract-violating *missing* flag would pass this mirror where the server's B2 rule fails absent data. Unreachable through the current wire (both flags are server-synthesized booleans, never omitted) and invisible to the fixture (no flag thresholds captured) — but it is part of this mirror's mental model, not a drift bug.
