...
1. **Container — flat `activities[].days[]`, one entry per 24h-block in the horizon (`days[i].dayIndex === i`).** With the current Meteosource `flexi` horizon (~164 hours) that is **7 entries, the last partial** — but the count is derived from `hours.length` (`floor((hours.length - 1) / 24) + 1`), **not hardcoded to 7**, so a shorter provider horizon yields fewer buckets. Dense, not sparse: a day with no qualifying window still occupies its slot, so the client indexes by position without scanning `dayIndex`. There is no nested `week` object — "day" stays derived (`floor(index / 24)`), consistent with [ADR-0003](0003-seven-day-horizon-flat-hours-day-buckets.md)'s flat-hours/derived-grouping rule and its **168-is-a-ceiling-not-a-count** principle.
...
**Consequences:** the per-day-result key order is pinned `dayIndex, rating, startIndex, endIndex, duration`, and the activity-object key order is `activityId, label, displayMetrics, days` — the golden snapshot in `tests/server/rating.test.js` reworks to assert these, plus that `days.length === floor((hours.length - 1) / 24) + 1` and that `days[i].dayIndex === i` (it must **not** hardcode `7` or `168` — per [ADR-0003](0003-seven-day-horizon-flat-hours-day-buckets.md), the horizon is provider-determined). `evaluateAll` returns the day-bucketed result described in [ADR-0003](0003-seven-day-horizon-flat-hours-day-buckets.md); the route layer shapes it into the wire form above. The iOS #5a decoder is now unblocked — it decodes `days[]` as a variable-length array (read `days.length`; do not assume 7) with an optional window per element, and reads `days[0]` for the card. The **Daily Digest** (per ADR-0003) slices day 0 — i.e. reads `days[0]` — before notifying. The POST request body (`{ lat, lon, activities }`) and its validation errors are **not** decided here; that is [ADR-0002](0002-activity-agnostic-engine.md)'s domain. Error mapping (400/502/500) is unchanged.
