# Meteosource — API reference (flexi tier)

Trimmed for agent lookup — read this first. Full raw export: [`openapi.json`](openapi.json) in this dir (the interactive OpenAPI/Swagger doc pulled from the Meteosource dashboard, saved 2026-07-20) — drop to it only when this summary doesn't answer the question.

## What we call

- Endpoint: `GET https://www.meteosource.com/api/v1/flexi/point` (`src/weather/fetch.js`)
...
- The `flexi` (paid) tier returns ~161–168 hourly entries — the 7×24 ceiling `parse.js` slices to (ADR-0003). `/free/` caps at 24h and is unusable for our 7-day horizon.
