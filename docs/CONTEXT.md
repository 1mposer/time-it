...
**Pursuit**:
The real-world outdoor thing a user does — Cycling, Running, Stargazing, Fishing, and whatever else a user authors from scratch. A **Pursuit** ships as one or more **Activities** that the engine actually evaluates. The **Lite / Pro** tier split is the usual reason one Pursuit ships as multiple Activities (e.g. a future **Fishing Pro** alongside Fishing Lite).

**Activity**:
The entity the decision engine evaluates — a configuration with a client-authored `id`, **Threshold** profile, `displayMetrics`, and an optional time-of-day **Window**. Activities are **caller-supplied**: the client authors them and sends them in the `POST /api/v1/rating` body; the engine holds **no** activity list and is activity-agnostic ([ADR-0002](adr/0002-activity-agnostic-engine.md), [ADR-0005](adr/0005-custom-activity-request-schema.md)). One **Pursuit** may be authored as multiple Activities; each is independently rated and rendered as its own iOS dashboard card. (The engine has never held a curated list; the client's own Template catalog was removed 2026-09-01 — every Activity is authored from scratch in the iOS wizard.)
...
_Avoid_: the word "threshold" in on-screen copy — tester evidence says users don't understand it; screens use plain language (owner ruling 2026-08-22). "Threshold" stays the domain term in docs, code, and the wire contract.
...
**Range**:
The from→to time a user sets when authoring or editing an **Activity** (e.g. Cycling 6am→9am) — the only hours the engine evaluates for that Activity each day. Every **Rating**, **Digest** line, and **Perfect-window alert** concerns the user's Range; nothing outside it is rated or alerted on. **Mandatory in the authoring UI** (range-first ruling): a range-less Activity is **dormant** — stored locally, excluded from every request and snapshot (the spec 14 dormancy model); the editor's Range section is unconditional — every save confirms a range, and dormancy ends only through that confirmation. Carried on the wire as the optional **time-of-day window** (next entry) — mandatory-Range is a client UI rule, not a contract change. Ranges are **whole-hour** (`localHour` is an integer; minute granularity would be an engine + wire change). A Range whose *from* is later than its *to* wraps midnight and makes the Activity **nocturnal**.
_Avoid_: framing the product as whole-day serendipitous discovery ("we find your moment") — that model was discarded (human audit 2026-08-11); its only descendant is the deferred, unbuilt client-side "surprise notification."
_Avoid_: the word "range" in **new** on-screen copy — tester evidence says users don't understand it; plain language only (owner ruling 2026-08-22, the onboarding-v2 vocabulary rule). "Range" stays the domain term in docs and code. Shipped surfaces still say "range" until the parked rework ([ROADMAP §Deferred](issues/ROADMAP.md)).

**Time-of-day window**:
An optional per-**Activity** `{ startHour, endHour }` (integers `0..23`, **forecast-location local** hours, half-open `[startHour, endHour)`) sent in the request that restricts evaluation to those hours each day. Absent = whole day; `startHour < endHour` = same-day; `startHour > endHour` = **midnight-wrap (nocturnal)**, the only nocturnal signal, which licenses the night-stitch; `startHour === endHour` is rejected. The client sends raw local hours and does **zero** timezone math — the time-boundary module tags each hour's `localHour` and the engine compares integers. See [ADR-0005](adr/0005-custom-activity-request-schema.md). *(Product-level term: **Range** — see above; the mandatory-in-UI rule, whole-hour constraint, and build-state caveat live there.)*
...
**Session**:
The user's actual outdoor activity time (e.g., Abdulla's 3-hour Sunday cycling block). A **Session** is shorter than or equal to a **Window** — the **Window** says when conditions are right within the Activity's **Range**; the **Session** is when the user actually goes out.
...
- A **User** authors **Activities** client-side (in the iOS wizard) and the iOS app sends them in the `POST /api/v1/rating` body; the backend holds no activity list.
