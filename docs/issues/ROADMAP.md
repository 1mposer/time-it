...
| 8 | #6c/#6d live acceptance (checklist: [push-client spec §6](current/implement-spec-push-client.md)) | both | **now — unblocked 2026-08-19** (item 4 + APNs entitlement done, release build on a real device); first box: the toggle-on `devices` row appears on Railway |
...
| 10 | **Beta feedback + test-build disclaimer** — [spec](current/implement-spec-beta-feedback.md); ships **inside** item 9's first build | both | server route ✅ done 2026-08-18 (`POST /api/v1/feedback` + `suggestions` table, 181 server tests green); Figma frames **owner-approved 2026-08-19** ([FIGMA.md §8](../design/FIGMA.md) — round-2 hand-edit is the code reference); iOS client build tasked 2026-08-19 ([ADR-0008](../adr/0008-figma-first-ui-gate.md) gate open) |
