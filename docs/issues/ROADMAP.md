...
| 10 | **Beta feedback + test-build disclaimer** — [spec](current/implement-spec-beta-feedback.md); ships **inside** item 9's first build | both | server route ✅ done 2026-08-18 (`POST /api/v1/feedback` + `suggestions` table, 181 server tests green); Figma frames (beta dashboard variants, light `340:3102` / dark `340:632`) awaiting owner approval; iOS client next — UI is Figma-first ([ADR-0008](../adr/0008-figma-first-ui-gate.md)) |
