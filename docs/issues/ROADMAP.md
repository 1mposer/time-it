...
| 11 | **Mobbin reference curation** — owner picks reference onboarding flows (e.g. Duolingo) via the connected Mobbin MCP and hands them to the design pass | owner | **done 2026-08-24** — two convergent research passes + owner pruning produced the approved reference board on the "Onboarding Flow" page ([FIGMA.md §9](../design/FIGMA.md)); 01 Welcome left deliberately unresearched (owner call) |
| 12 | **Onboarding v2 — design** ([FIGMA.md §9](../design/FIGMA.md) is the pass record) — a **new dedicated Figma page**, frames built **fresh** (§7/§8 and the existing Screens pages are never edited); flow: hobby pick (Templates as starting points) → **three one-tap personalization questions**, prefilled from the template — (1) "When do you usually [run]?" → sets the mandatory **Range** (the activity is born ranged and non-dormant — locked contract), (2)+(3) two comfort questions → the hobby's two most important thresholds — plus an optional **fine-tune offer** with an always-visible back-out (fine-tune's destination is a decision *inside this design pass*, not a promise of the old wizard screens) → **location ask** (one plain sentence of why; refusal falls into the city picker — the refusal path is a **first-class frame**) → **live verdict reveal** → one optional "Add another?" → dashboard. **Vocabulary rule:** "range" and "threshold" never appear on these screens — plain language only; strings live in Figma (design truth), never in docs | owner + agent | **in progress** — grey-box wireframes v1 + Rive asset briefs on the "Onboarding Wireframes" page (2026-08-24), **hi-fi v1 frames built 2026-09-21** in a row below them (12 light frames incl. Q2/Q3 and the 04b refusal frame — [FIGMA.md §9](../design/FIGMA.md) has the ids + the agent judgment calls to glance at); **Rive ruled out of scope 2026-09-21 (owner)** — the four motion slots ship as static poses, the asset briefs stay on the page as history; owner approval of frames per [ADR-0008](../adr/0008-figma-first-ui-gate.md) closes it |
...
| 15 | **Interim polish round** (owner-directed 2026-09-01, separate lane — items 11–13 untouched): template flow removed entirely (empty first launch + Add-Activity hero, "+" straight into the wizard, one-time legacy-seed launch purge for existing installs), card rebuilt to the **main look** for all states (blue Range chip, 2-label axis, chips everywhere, red-slice-only null verdict — phrase behind the Show-phrases toggle); Figma pass + approvals: [FIGMA.md §10](../design/FIGMA.md) | both | **code + docs done 2026-09-01**; remaining: owner archives + uploads the build (Xcode) and invites **2 new internal testers** (known people, ASC team → internal group; external path stays HELD per item 9) |
...
| Seed-threshold finalization (owner pass) — `SeedTemplates.swift` was **deleted** in the item-15 template removal (2026-09-01); the provisional values live in git history and the test/preview fixtures (`PreviewFixtures`, `Fixtures`) | rides the onboarding v2 build (item 13) — the three prefilled questions re-home these defaults client-side |
