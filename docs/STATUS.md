...
> **Last updated: 2026-09-21** — onboarding-v2 hi-fi v1 frames built (light, 12 frames — [FIGMA.md §9](design/FIGMA.md)), awaiting owner approval; Rive ruled out of scope for this phase. Card v2 score-ring + detail v3 hour-stepper frames built ([FIGMA.md §11](design/FIGMA.md)) — the 1–100 day score is a design placeholder with no backend yet ([ROADMAP item 16](issues/ROADMAP.md)). Interim polish round ([ROADMAP item 15](issues/ROADMAP.md)) code + docs done: template flow removed (empty first launch → Add-Activity hero, wizard-only add, legacy-seed launch purge), card rebuilt to the main look ([FIGMA.md §10](design/FIGMA.md)); awaiting the owner's archive/upload + 2 new internal testers. Events of record: [ROADMAP](issues/ROADMAP.md). This file is deliberately one screen: a snapshot only.
...
- **NOW — three threads:** (a) **ship the interim polish build** ([ROADMAP item 15](issues/ROADMAP.md)): owner archives + uploads in Xcode, invites 2 new internal testers (known people, internal group only); (b) **onboarding-v2 design phase** ([ROADMAP items 11–12](issues/ROADMAP.md)): reference board approved + grey-box wireframes built (2026-08-24) + **hi-fi v1 frames built (2026-09-21 — [FIGMA.md §9](design/FIGMA.md), Rive out of scope)**; next: owner approval ([ADR-0008](adr/0008-figma-first-ui-gate.md) gate) → item 13 build; (c) **finish the #6c/#6d acceptance boxes** ([checklist: push-client spec §6](issues/current/implement-spec-push-client.md), ROADMAP item 8) — remaining boxes are time-gated: digest 6–11 band, dedup silence, upgrade push, far-day, pruning, toggle-off re-verify under the never-erase code — plus the spec-14 dormancy box (not time-gated). **Awaiting owner review (flagged, not decided):** spec 14's I3 (provisional phrase strings). *(I2 — weather rows hidden while nothing is live — resolved 2026-09-01 by the approved First Launch frame, [FIGMA.md §10](design/FIGMA.md).)*
- **NEXT (ship order — ROADMAP has the full table):** onboarding-v2 **build** (item 13, Figma-gated) → **watched-session dad-test round** (item 14) → the item-9 external path unfreezes. Day-score backend (item 16) waits on owner approval of the §11 frames. Item 10's live boxes are ticked except the non-204/429 UX ([spec §5](issues/current/implement-spec-beta-feedback.md)).
