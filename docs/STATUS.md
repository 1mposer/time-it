...
> **Last updated: 2026-07-03** — **Rebuild Phase 1 + Phase 2 merged to `main` (`9a9c60b`); the iOS #5a spec + shared #5 design-decisions doc reconciled to the shipped contract (`2a5a50b`). #5a is ready to build.** Request flip shipped: `POST /api/v1/rating` with body `{ lat, lon, activities[] }`, caller-supplied `evaluateAll(hours, activities)` (engine holds no list; `src/activities/*` deleted), atomic structured `{ errors[] }` validation (`validateRatingRequest.js`) + live/coming-soon `metricCatalog.js` (coming-soon hard-400), `timeBoundary` now tags `localHour`, and the wrap-gated **night-stitch** (nocturnal activities bucket by night; per-activity `days.length`). The ultra-review (2026-06-30) surfaced two typed-error regressions — both fixed and test-pinned in `f983179` (adapter `forecastStart` shape-guard → `UpstreamError`/502; `express.json()` malformed-JSON/oversized body → uniform `{ errors[] }` 400/413). Full suite green (**110**). **Next: build iOS #5a from [`implement-spec-issue-5a-ios-core.md`](issues/current/implement-spec-issue-5a-ios-core.md)** (POST seed Templates, decode `days[]`/`timezone`, render card + 7-day timeline). Prior (2026-07-03): rebuild merged to `main`; #5a/#5 specs reconciled. Prior (2026-06-24): Phase 2 implemented + merged. Prior (2026-06-22): Phase 1 (7-day day-bucketed `days[]`/`timezone` output) implemented + merged. Prior (2026-06-20): ADR-0005 request schema pinned. Prior (2026-06-19): day-bucketing via provider IANA `timezone`, `flexi` horizon verified (168 = ceiling), B2 resolved.
...
> **▶ NEXT (start here, fresh session): build iOS #5a from [`implement-spec-issue-5a-ios-core.md`](issues/current/implement-spec-issue-5a-ios-core.md).** The spec is reconciled to the shipped contract (`2a5a50b`) — a stale-less entry point: POST the two seed Templates (Cycling + Fishing-lite, live metrics only), decode per-activity `days[]` + top-level `timezone`, render the card (soonest-actionable day) + the 7-day timeline detail; no accounts, no tab bar, no `-pro`/PRO. Read that spec + the shared [`design-decisions-issue-5.md`](issues/current/design-decisions-issue-5.md) (also reconciled) + [`ios/guidelines/Guidelines.md`](../ios/guidelines/Guidelines.md). **Backend is done and merged to `main`:** the `GET→POST` contract flip, caller-supplied `evaluateAll(hours, activities)`, ADR-0005 validation + `metricCatalog.js`, and the wrap-gated **night-stitch** are all built; `src/activities/*` removed; full suite green (**110**). The ultra-review ran (2026-06-30); its two typed-error regressions are fixed and pinned in `f983179` (adapter `forecastStart` guard → 502; body-parser 400/413 in the uniform `{ errors[] }` envelope — documented in the CLAUDE.md error table + ADR-0005 §6). **Cleanly parallel / not yet built:** `GET /api/v1/metrics` route (needs ADR-0006; reads the existing `metricCatalog.js`); client-side stale-activity reconciliation (§4); ADR-0005 error-`code` enum; seed Template threshold values (#5b). **Advisory (not blockers):** the 2026-06-30 deep-modules review flagged 3 internal-seam deepenings (fold metric catalog into the parser field map; keep the tagging seam off wire-bound hours; drop a vestigial field) — module shape, not the wire contract; defer or invest, independent of #5a. **Known limitation (not a bug):** DST *inside* a time-of-day window is untested (Asia/Dubai has no DST) — do not assert DST-correctness for windows.

- **Current backend (through #10):** stable, tested, buildable.
- **iOS #5a (core app):** **scoped & unblocked (2026-06-20); spec reconciled & ready to build (2026-07-03, `2a5a50b`).** Decision: **#5a ships core read-only first** — decode `days[]`/`timezone`, render card + soonest-actionable fallback + 7-day timeline. The build brief is [`implement-spec-issue-5a-ios-core.md`](issues/current/implement-spec-issue-5a-ios-core.md) (no longer stale — matches ADR-0001/0004/0005 + CLAUDE.md), with the shared [`design-decisions-issue-5.md`](issues/current/design-decisions-issue-5.md) reconciled alongside it. **Note (post-Phase-2):** the backend is **POST-only** with **caller-supplied** activities — there is no curated/hardcoded list to read against, so even the read-only #5a must encode a `POST` body (seed Templates client-side; #5a ships Cycling + Fishing-lite, live metrics only). Full authoring UI + metric-picker still defer to the **#5b wave** (resolves grill PENDING #2). Wire shape pinned in [ADR-0004](adr/0004-day-bucketed-rating-wire-shape.md) incl. the 2026-06-20 amendments (per-activity `days.length`, nocturnal tail/label, per-hour shape + `hour` dropped); decode against variable-length `days[]` (read `days.length` per activity; never hardcode 7). **Night-stitch is now live** (Phase 2): a nocturnal Activity (wrapped `window`) returns stitched cross-midnight nights, so the Stargazing card no longer fragments at calendar-midnight.
