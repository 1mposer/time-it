...
> **Last updated: 2026-09-21** — onboarding-v2 hi-fi v1 frames built (light, 12 frames — [FIGMA.md §9](design/FIGMA.md)), awaiting owner approval; Rive ruled out of scope for this phase. Interim polish round ([ROADMAP item 15](issues/ROADMAP.md)) code + docs done: template flow removed (empty first launch → Add-Activity hero, wizard-only add, legacy-seed launch purge), card rebuilt to the main look ([FIGMA.md §10](design/FIGMA.md)); awaiting the owner's archive/upload + 2 new internal testers. Events of record: [ROADMAP](issues/ROADMAP.md). This file is deliberately one screen: a snapshot only.
